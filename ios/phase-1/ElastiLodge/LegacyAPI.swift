...
class LegacyAPI : ElastiLodgeAPI{
    ...
    init(hotelId : String) {
        ...
        //
        // TODO : change the URL with your API Gateway URL
        //
        let detailsDataURL = "https://zd646axw51.execute-api.eu-west-1.amazonaws.com/Prod/hotels/\(hotelId)"
    ...
    func loadLodgeDetails(detailsUrl : String) -> Void {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            do {
                ...
                self.jsonDetails = jsonData["hotel"] as? [String:Any]
    ...
    var amenities: String? {
        guard let am = jsonDetails?["amenities"] as? [String] else {
            fatalError("Can not parse amenities")
        }
        return am.joined(separator: "\n")
    }
    func restaurant(_ atIndex : Int) -> [String]? {
        guard let r = jsonDetails?["restaurants"] as? [[String:String]] else {
            fatalError("Can not parse restaurants")
        }
        if atIndex < r.count {
            return [ r[atIndex]["name"] ?? "", r[atIndex]["description"] ?? ""]
        } else {
            return nil
        }
    }
